...
CREATE TABLE dataclass_relationship.gene_alleleV2
AS
SELECT DISTINCT *
FROM (
	SELECT allele.uniquename AS allele_id,
...
JOIN dataclass.alleleV2 ON dataclass.alleleV2.id = subquery.allele_id
;

ALTER TABLE dataclass_relationship.gene_alleleV2 ADD PRIMARY KEY (gene_id, allele_id, is_construct);

ALTER TABLE dataclass_relationship.gene_alleleV2
    ADD CONSTRAINT gene_allele_fk1
    FOREIGN KEY (gene_id) REFERENCES dataclass.geneV2 (id);
ALTER TABLE dataclass_relationship.gene_alleleV2
    ADD CONSTRAINT gene_allele_fk2
    FOREIGN KEY (allele_id) REFERENCES dataclass.alleleV2 (id);

CREATE INDEX gene_allele_idx1 ON dataclass_relationship.gene_alleleV2 (gene_id);
CREATE INDEX gene_allele_idx2 ON dataclass_relationship.gene_alleleV2 (allele_id);
CREATE INDEX gene_allele_idx3 ON dataclass_relationship.gene_alleleV2 (is_construct);
