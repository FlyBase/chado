...
JOIN LATERAL (
	...
	JOIN synonym
		USING (synonym_id)
	WHERE feature_synonym.is_current = TRUE
		AND feature_synonym.is_internal = FALSE
		AND synonym.type_id = 60037 --fullname
		AND feature_synonym.feature_id = gene.feature_id
	LIMIT 1
) AS s_fullname ON TRUE
--Add symbol
JOIN LATERAL (
	SELECT *
	FROM feature_synonym
	...
	WHERE feature_synonym.is_current = TRUE
		AND feature_synonym.is_internal = FALSE
		AND synonym.type_id = 59978 --symbol
		AND feature_synonym.feature_id = gene.feature_id
	LIMIT 1
) AS s_symbol ON TRUE
