...
/**
This function is the same as above, except only returning a count of pubs that are of type a specific type

*/
CREATE OR REPLACE FUNCTION flybase.pub_count_by_type(id text, pub_type text)
    RETURNS bigint AS
$$
DECLARE
    count        bigint = 0;
...
BEGIN
    ...
    select count(p.*)
      ...
      where obj.uniquename = %4$L
        ...
        and upper(flybase.data_class(p.uniquename)) = %5$L
        and c.name = %6$L
    ;', object_table, linker_table, linker_field, id, 'FBRF', pub_type)
        INTO count;
    RETURN count;
END
$$ LANGUAGE plpgsql stable;
COMMENT ON FUNCTION flybase.pub_count_by_type(text, text) IS 'Given a FlyBase ID, returns a count of FlyBase pub records of the specified type directly associated with it.';
