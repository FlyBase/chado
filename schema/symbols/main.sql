    ...
    if upper(flybase.data_class(id)) = 'FBST' and upper(synonym_type) = 'SYMBOL' then
      ...
      limit 1;', id)
        ...
        case upper(flybase.data_class(id))
            when 'FBGG' then linker_table = 'grp_synonym';
                             object_table = 'grp';
                             linker_field = 'grp_id';
            when 'FBSN' then linker_table = 'strain_synonym';
                             object_table = 'strain';
                             linker_field = 'strain_id';
            when 'FBTC' then linker_table = 'cell_line_synonym';
                             object_table = 'cell_line';
                             linker_field = 'cell_line_id';
            when 'FBHH' then linker_table = 'humanhealth_synonym';
                             object_table = 'humanhealth';
                             linker_field = 'humanhealth_id';
            when 'FBLC' then linker_table = 'library_synonym';
                             object_table = 'library';
                             linker_field = 'library_id';
            else linker_table = 'feature_synonym';
